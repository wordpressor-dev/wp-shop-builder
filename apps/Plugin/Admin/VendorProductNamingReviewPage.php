...
use Closure;
use Throwable;
use WPShop\App\Plugin\ProductManager\Naming\VendorProductNamingReviewRow;
...
final class VendorProductNamingReviewPage implements SubmenuPageInterface
{
    private const REPORT_META_KEY = 'wp_shop_pm_vendor_naming_review_v4';
    private const STATE_META_KEY = 'wp_shop_pm_vendor_naming_review_state_v4';

    /**
     * @param Closure(string, mixed...): mixed $call
     */
    public function __construct(
        private readonly VendorProductNamingReviewService $review,
    ...
    {
        return 'wp-shop-builder-vendor-naming-review-v4';
    }

    public function title(): string
    {
        return 'Vendor Naming Review V4';
    }
    ...
    {
        $action = $this->posted('wp_shop_pm_vendor_naming_review_action');
        ...
        if ($action === 'review_start') {
            ...
            if ($state['status'] !== 'RUNNING') {
                $error = 'No running Vendor Naming Review V4 was found.';
            } else {
                [$state, $message, $error] = $this->processNextBatch($state);
                $autoContinue = $error === '' && $state['status'] === 'RUNNING';
            }
        ...
        echo '<div class="wrap">';
        echo '<h1>WP Shop Product Manager — Vendor Naming Review V4</h1>';
        echo '<p><strong>V4 / DRY RUN ONLY / PRODUCT WRITES = 0.</strong> Processes only Vendor products that remain REVIEW in the current naming rules. It combines the current H1, Vendor Sales Page H1/title, ZIP header identity and current TranslatePress title state. Sales-page evidence is advisory and can never write product data.</p>';
        echo '<p><strong>Actions:</strong> KEEP = official page supports current H1; RENAME_CANDIDATE = official page supports the ZIP/audit candidate; MANUAL_REVIEW = evidence is inconclusive; PACKAGE_PROBLEM = ZIP/package evidence must be fixed first.</p>';
        ...
        if ($error !== '') {
            echo '<div class="notice notice-error"><p><strong>VENDOR NAMING REVIEW V4 ERROR:</strong> '
                . $this->escape($error)
                . '</p></div>';
        }
        ...
        if ($rows !== []) {
            echo '<div class="postbox" style="max-width:1800px;padding:18px 20px;">';
            echo '<h2 style="margin-top:0;">V4 Review Results</h2>';
            ...
            foreach ($rows as $row) {
                ...
                echo '<td><code>' . $this->escape((string) ($row['salesPageStatus'] ?? '')) . '</code></td>';
    ...
    public function exportCsv(): void
    {
        ...
        ($this->call)(
            'check_admin_referer',
            'wp_shop_pm_export_vendor_naming_review_v4',
            '_wpnonce'
        );

        $report = $this->loadReport();
        $filename = 'wp-shop-vendor-product-naming-review-v4-'
    ...
    private function processNextBatch(array $state): array
    {
        ...
        if ($finished) {
            ...
            $message = 'VENDOR NAMING REVIEW V4 = READY';
        } else {
            $state['status'] = 'RUNNING';
            $message = 'VENDOR NAMING REVIEW V4 BATCH = SAVED';
        }
    ...
    /**
     * @param list<VendorProductNamingReviewRow> $rows
     */
     ...
     * @return array{scanned:int,keep:int,rename:int,manual:int,package:int}
     */
    ...
    /**
     * @param array<string, int|string> $state
     * @param array{scanned:int,keep:int,rename:int,manual:int,package:int} $summary
     */
    private function renderProgress(array $state, array $summary): void
    {
        ...
        echo '<p><strong>VENDOR NAMING REVIEW V4 = '
        ...
        echo '<p><strong>PRODUCT WRITES = 0</strong> &nbsp; KEEP = '
            ...
            . ' &nbsp; PACKAGE_PROBLEM = '
            . $this->escape((string) $summary['package'])
            . '</p>';
        echo '</div>';
    }
    ...
    private function renderControls(array $state): void
    {
        ...
        echo '<h2 style="margin-top:0;">Vendor REVIEW evidence pass</h2>';
        echo '<p>Recommended batch size is 5 because Vendor Sales Pages are fetched with a short timeout. Start rebuilds this administrator\'s V4 report from the beginning.</p>';
        echo '<form method="post">';
        $this->nonceField();
        echo '<input type="hidden" name="wp_shop_pm_vendor_naming_review_action" value="review_start">';
        ...
        echo '<button type="submit" class="button button-primary">Запустить Vendor Naming Review V4 — без записи</button>';
        echo '</form>';

        if ($state['status'] === 'RUNNING') {
            echo '<form method="post" style="margin-top:12px;">';
            $this->nonceField();
            echo '<input type="hidden" name="wp_shop_pm_vendor_naming_review_action" value="review_resume">';
            echo '<button type="submit" class="button button-secondary">Продолжить Vendor Naming Review V4</button>';
            echo '</form>';
        }
    ...
    private function renderExport(): void
    {
        ...
        echo '<input type="hidden" name="action" value="wp_shop_pm_export_vendor_naming_review_v4">';
        echo '<button type="submit" class="button button-secondary">Export Vendor Naming Review V4 CSV</button>';
        echo '</form>';
    }

    private function renderAutoContinue(): void
    {
        echo '<form id="wp-shop-vendor-naming-review-v4-next" method="post" style="display:none;">';
        $this->nonceField();
        echo '<input type="hidden" name="wp_shop_pm_vendor_naming_review_action" value="review_next">';
        echo '</form>';
        echo '<script>';
        echo 'window.setTimeout(function(){var f=document.getElementById("wp-shop-vendor-naming-review-v4-next");if(f){f.submit();}},1200);';
        echo '</script>';
    }
    ...
    private function checkNonce(): void
    {
        ($this->call)(
            'check_admin_referer',
            'wp_shop_pm_vendor_naming_review_v4',
            '_wpnonce'
        );
    }

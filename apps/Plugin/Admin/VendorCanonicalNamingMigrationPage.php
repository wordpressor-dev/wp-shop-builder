...
final class VendorCanonicalNamingMigrationPage implements SubmenuPageInterface
{
    private const REPORT_META_KEY =
        'wp_shop_pm_vendor_canonical_naming_migration_v1';
    private const STATE_META_KEY =
        'wp_shop_pm_vendor_canonical_naming_migration_state_v1';
    ...
    {
        return 'wp-shop-builder-vendor-canonical-naming-migration';
    }

    public function title(): string
    {
        return 'Vendor Canonical Naming Migration';
    }
    ...
    {
        $action = $this->posted('wp_shop_pm_vendor_canonical_action');
        ...
        if ($action === 'migration_start') {
            ...
            if ($state['status'] !== 'RUNNING') {
                $error = 'No running Vendor Canonical Naming Migration was found.';
            } else {
                [$state, $message, $error] = $this->processNextBatch($state);
                $autoContinue = $error === '' && $state['status'] === 'RUNNING';
            }
        ...
        echo '<div class="wrap">';
        echo '<h1>WP Shop Product Manager — Vendor Canonical Naming Migration</h1>';
        echo '<p><strong>APPROVED MANIFEST = 13 PRODUCTS.</strong> This migration changes only approved Vendor H1 values. It preserves the slug, rechecks Vendor classification and Sales Page, validates current H1, and requires an exact TranslatePress EN title after every write. If TranslatePress remap fails, the H1 is rolled back.</p>';
        echo '<p><strong>NOT INCLUDED:</strong> AutoPoly remains manual. wpDiscuz remains blocked until its package TYPE_MISMATCH is resolved.</p>';
        ...
        if ($error !== '') {
            echo '<div class="notice notice-error"><p><strong>VENDOR CANONICAL NAMING MIGRATION ERROR:</strong> '
                . $this->escape($error)
                . '</p></div>';
        }
    ...
    private function renderControls(array $state): void
    {
        ...
        if ($state['status'] !== 'RUNNING') {
            ...
            $this->nonceField();
            echo '<input type="hidden" name="wp_shop_pm_vendor_canonical_action" value="migration_start">';
            echo '<button type="submit" class="button button-primary">Apply TranslatePress-Aware Vendor Canonical Migration</button>';
            echo '</form>';
        } else {
            echo '<form method="post">';
            $this->nonceField();
            echo '<input type="hidden" name="wp_shop_pm_vendor_canonical_action" value="migration_resume">';
    ...
    private function processNextBatch(array $state): array
    {
        ...
        return [
            $state,
            $finished
                ? 'VENDOR CANONICAL NAMING MIGRATION = READY'
                : 'VENDOR CANONICAL NAMING MIGRATION BATCH = SAVED',
            '',
        ];
    }
    ...
    {
        echo '<form id="wp-shop-vendor-canonical-next" method="post" style="display:none;">';
        $this->nonceField();
        echo '<input type="hidden" name="wp_shop_pm_vendor_canonical_action" value="migration_next">';
        echo '</form>';
        echo '<script>';
        echo 'window.setTimeout(function(){var f=document.getElementById("wp-shop-vendor-canonical-next");if(f){f.submit();}},1200);';
        echo '</script>';
    }
    ...
    private function checkNonce(): void
    {
        ($this->call)(
            'check_admin_referer',
            'wp_shop_pm_vendor_canonical_naming',
            '_wpnonce'
        );
    }
